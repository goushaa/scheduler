...
void SRTN_Algo()
{
    while (remainingProcesses > 0)
    {
        ...
        if (currentPCB->start == -1)
        {
            ...
            fprintf(sl,"At time %d process %d started arr %d total %d remain %d wait %d\n", prevTime, currentPCB->fileInfo.id, currentPCB->fileInfo.arrival, currentPCB->fileInfo.runtime, rem, currentTime -(currentPCB->fileInfo.runtime-currentPCB->remainingTime) - currentPCB->fileInfo.arrival);
        }
        else
            fprintf(sl,"At time %d process %d resumed arr %d total %d remain %d wait %d\n", prevTime, currentPCB->fileInfo.id, currentPCB->fileInfo.arrival, currentPCB->fileInfo.runtime, rem, currentTime -(currentPCB->fileInfo.runtime-currentPCB->remainingTime) - currentPCB->fileInfo.arrival);
        currentPCB->state = 0;
